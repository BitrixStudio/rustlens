...
pub async fn run(
    ...
    while let Some(cmd) = cmd_rx.recv().await {
        ...
        let res = match cmd {
            ...
            DbCmd::LoadTablePage { schema, table, page, page_size } => {
                match crate::db::postgres::load_table_page(&pool, &schema, &table, page, page_size).await {
                    Ok((columns, rows)) => {
                        let _ = evt_tx.send(DbEvt::QueryResult {
                            columns,
                            rows,
